module.exports = function (PersistObjectTemplate) {
    ...
    PersistObjectTemplate.persistSaveMongo = function (obj, promises, masterId, idMap, txn, logger) {
        ...
        if (idMap[id.toString()]) {
            (logger || this.logger).debug({ component: 'persistor', module: 'update.persistSaveMongo', activity: 'processing' }, 'Duplicate processing of ' + obj.__template__.__name__ + ':' + id.toString());
            return idMap[id.toString()];
        }
        (logger || this.logger).debug({ component: 'persistor', module: 'update.persistSaveMongo', activity: 'save' }, 'Saving ' + obj.__template__.__name__ + ':' + id.toString() + ' master_id=' + masterId);
