module.exports = function (PersistObjectTemplate) {
            ...
            return this.getPOJOFromMongoQuery(template, subQuery.query, options, logger).then(function (pojos) {
                ...
                for (var ix = 0; ix < pojos.length; ++ix) {
                    ...
                    if (!idMap[pojos[ix]._id.toString()]) {
                        ...
                        this.getTemplateFromMongoPOJO(pojos[ix], topType, promises, { type: topType }, idMap, {}, null, null, isTransient);
                    }
    ...
    PersistObjectTemplate.getTemplateFromMongoPOJO = function (pojo, template, promises, defineProperty, idMap, cascade, establishedObj, specificProperties, isTransient, logger) {
        ...
        for (var prop in props) {
            ...
            if (type == Array) {
                ...
                if (!defineProperty.of.__collection__)
                        ...
                        if (pojo[prop][ix]) {
                            ...
                            if (typeof (pojo[prop][ix]) == 'string') {
                                ...
                                obj[prop][ix] = idMap[pojo[prop][ix]._id.toString()] ||
                                    this.getTemplateFromMongoPOJO(pojo[prop][ix], defineProperty.of, promises, defineProperty, idMap, cascadeFetchProp, null, null, isTransient, logger);
                            }
                        }
                        else
                            obj[prop][ix] = null;
                    ...
                    if (doFetch) {
                        ...
                        (logger || this.logger).debug({ component: 'persistor', module: 'query.getTemplateFromMongoPOJO', activity: 'pre' }, 'fetching ' + prop + ' cascading ' + JSON.stringify(cascadeFetch) + ' ' + JSON.stringify(query) + ' ' + JSON.stringify(options));
                        self = this;
                        (function () {
                            var closureProp = prop;
                            var closureOf = defineProperty.of;
                                ...
                                if (closureIsSubDoc) {
                                    ...
                                    for (var ix = 0; ix < pojos.length; ++ix) {
                                        ...
                                        if (!idMap[pojos[ix]._id.toString()]) {
                                            ...
                                            self.getTemplateFromMongoPOJO(pojos[ix], topType, promises, { type: topType }, idMap, {}, null, null, isTransient, logger);
                                        }
                                        ...
                                        for (var jx = 0; jx < subPojos.length; ++jx)
                                            ...
                                            obj[closureProp].push((!closureCascade && idMap[subPojos[jx]._id.toString()]) ||
                                                self.getTemplateFromMongoPOJO(subPojos[jx], closureDefineProperty.of, promises, closureDefineProperty, idMap, closureCascade, null, null, isTransient, logger));
                                    }
                                }
                                else
                                    for (ix = 0; ix < pojos.length; ++ix) {
                                        ...
                                        obj[closureProp][ix] = idMap[pojos[ix]._id.toString()] ||
                                            self.getTemplateFromMongoPOJO(pojos[ix], closureDefineProperty.of, promises, closureDefineProperty, idMap, closureCascade, null, null, isTransient, logger);
                                    }
                ...
                if (type.__collection__ == collection && !isCrossDocRef) {
                    ...
                    if (pojo[prop]) {
                        ...
                        if (typeof (pojo[prop]) == 'string') {
                            ...
                            options = defineProperty.queryOptions || {};
                            cascadeFetchProp = this.processCascade(query, options, cascadeFetch, null, defineProperty.fetch);
                            obj[prop] = idMap[pojo[prop]._id.toString()] || this.getTemplateFromMongoPOJO(pojo[prop], type, promises, defineProperty, idMap, cascadeFetchProp, null, null, isTransient, logger);
                        }
                    }
                    else
                        obj[prop] = null;
                    ...
                    if (idMap[foreignId]) {
                        ...
                        if (doFetch) {
                            ...
                            if (foreignId) {
                                ...
                                options = {};
                                (logger || this.logger).debug({ component: 'persistor', module: 'query.getTemplateFromMongoPOJ', activity: 'processing' }, 'fetching ' + prop + ' cascading ' + JSON.stringify(cascadeFetch));
                                self = this;
                                (function () {
                                    var closureProp = prop;
                                    var closureType = type;
                                    var closurePersistorProp = persistorPropertyName;
                                    var closureCascade = self.processCascade(query, options, cascadeFetch, (schema && schema.parents) ? schema.parents[prop].fetch : null, defineProperty.fetch);
                                    var closureDefineProperty = defineProperty;
                                            ...
                                            if (pojos.length > 0) {
                                                if (closureIsSubDoc) {
                                                    ...
                                                    if (!idMap[pojos[0]._id.toString()]) {
                                                        ...
                                                        self.getTemplateFromMongoPOJO(pojos[0], topType, promises, { type: topType }, idMap, {}, null, null, isTransient);
                                                    }
                                                    // Get actual sub-doc since it may not yet be available from processing doc
                                                    var subDocPojo = self.getPOJOSFromPaths(closureType, this.createSubDocQuery(null, closureType, logger).paths, pojos[0], { _id: closureForeignId })[0];
                                                    ...
                                                    if (subDocPojo && subDocPojo._id) {
                                                        if (!idMap[subDocPojo._id.toString()]) {
                                                            self.getTemplateFromMongoPOJO(subDocPojo, closureType, promises, closureDefineProperty, idMap, closureCascade, null, null, isTransient, logger);
                                                        }
                                                    }
                                                    else {
                                                        (logger || this.logger).debug({ component: 'persistor', module: 'query.getTemplateFromMongoPOJO', activity: 'processing' }, 'Orphaned subdoc on ' + obj.__template__.__name__ + '[' + closureProp + ':' + obj._id + '] ' +
                                                            'foreign key: ' + closureForeignId + ' query: ' + JSON.stringify(this.createSubDocQuery(null, closureType, logger)));
                                                    }
                                                }
                                                else if (!idMap[pojos[0]._id.toString()])
                                                    self.getTemplateFromMongoPOJO(pojos[0], closureType, promises, closureDefineProperty, idMap, closureCascade, null, null, isTransient, logger);
                                            }
                                            obj[closureProp] = idMap[closureForeignId];
                                            obj[closurePersistorProp].isFetched = true;
                                            obj[closurePersistorProp] = copyProps(obj[closurePersistorProp]);
            ...
            else if (typeof (pojo[prop]) != 'undefined') {
                if (type == Date)
